...
exports.sendTestEmail = (0, https_1.onCall)({
    ...
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated to send test emails');
    }
    const { to, subject, plainContent, htmlContent, templateName } = request.data;
    // Validate inputs
    if (!to || !subject || !plainContent) {
        throw new https_1.HttpsError('invalid-argument', 'Missing required fields: to, subject, plainContent');
    }
