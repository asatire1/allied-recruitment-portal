...
export const cleanupExpiredBookingLinks = onSchedule(
  {
...
export const checkCandidateBookingExpiry = onCall<{ candidateId: string }>(
  {
    cors: true,
  },
  async (request) => {
    ...
    try {
      ...
      throw new HttpsError('internal', err.message || 'Error checking booking expiry')
    }
  }
)
...
export const triggerExpiredBookingCleanup = onCall(
  {
    cors: true,
  },
  async (request) => {
